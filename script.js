document.addEventListener("DOMContentLoaded", function() {
     ...
     */
    let pcOAuthClientIds = {"de": "florian.zacchero:6789"};
    let clientId = pcOAuthClientIds["de"];
    if (!clientId) {
        setErrorState(
    ...
    document
        ...
        .addEventListener("click", function() {
            let externalOrganizationId = document
                .querySelector("#org-id")
                .value.trim();
    ...
    function getEmbeddingPCEnv() {
        ...
        if (!result && window.location.search) {
            result =
                extractParams(window.location.search.substring(1))
                    .pcEnvironment || null;
        }
